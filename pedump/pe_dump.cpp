...
int BuildImportTable(PMEMORYMODULE module)
{
	...
	for ( ; !IsBadReadPtr(importDesc, sizeof(IMAGE_IMPORT_DESCRIPTOR)) && importDesc->Name; importDesc++ )
	{
		...
		HMODULE *p = (HMODULE*)realloc( module->modules, (module->numModules+1)*(sizeof(HMODULE)) );
		module->modules = p;
		...
		for (; *thunkRef; thunkRef++, funcRef++)
		{
			...
			{
				printf("iat %p is %s", funcRef, (LPCSTR)IMAGE_ORDINAL(*thunkRef));
...
HMEMORYMODULE MemoryLoadLibrary(const void *data)
{
	...
	result = (PMEMORYMODULE)malloc( sizeof(MEMORYMODULE) );
...
int main(int argc, char** argv)
{
	...
	if(argc!=2)
		printf("usage %s, <pe_file_name>", argv[1]);

	DllBody = ReadWholeFile( argv[2], dwFileSize );
